...
public class StatusBarUtil {
    ...
    public static int getStatusBarHeight(Context context) {
        try {
            ...
            Logout.d("getStatusBarHeight"+CommonUtil.dp2px(context,context.getResources().getDimensionPixelSize(x)));
            return CommonUtil.dp2px(context,24);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return CommonUtil.dp2px(context,24);
    ...
    public static void setStatusBar(Activity activity, boolean useThemestatusBarColor, boolean withoutUseStatusBarColor) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {//5.0及以上
            ...
            if (useThemestatusBarColor) {
                activity.getWindow().setStatusBarColor(activity.getResources().getColor(R.color.primgreen));
            } else {
